...
import {Label,Grid,Header, Button,Form} from 'semantic-ui-react'
...
export default class DraggableMarkerMapWithAutoFilledForm extends Component {
  state = {
  ...
  onMarkerInteraction3=()=> {
   ...
   // console.log('Finalposn: ', addressByMap);
    ...
    onInputChange=(e,{name,value})=>this.setState({
      ...this.state,
      [name]:value
    })
    ...
    onSubmit=()=>{
      // addressByMap
      const {addressByMap} = this.state
     console.log(addressByMap)
if(addressByMap!==''){
     returnLatLongFromAddress(addressByMap).then(
      ({lat,lng})=>
       this.setState( {
        center: {
          lat,
  ...
  render() {
    ...
    return ( // Important! Always set the container height explicitly
      <Grid columns={2} verticalAlign='middle'>
        ...
        <Grid.Column>
        <Header as='h2' color='teal'>PHYSICAL ADDRESS BY MAP: </Header>
        <Header> {addressByMap}</Header>
        <Button onClick={this.getIt}>Get IT</Button>
        <Header as='h2' color='teal'>Position Marker By Address Name Form Input: </Header>
        <Form onSubmit={this.onSubmit}>
          <Form.Input onChange={this.onInputChange}
          type='text' name='addressByMap' placeholder='Enter Name of Significant feature'/>
          <Button primary fluid>Position Warehouse Marker</Button>
        </Form>
        </Grid.Column>
      </Grid>
    )
